...
def init_path(checkpoint_dir, config_dir, size=512, old_version=False, preprocess='crop'):
    ...
    if old_version:
    ...
    elif len(glob.glob(os.path.join(checkpoint_dir, '*.safetensors'))) > 0:
        ...
        print("WARNING: The new version of model will be updated by safetensor, you may need to download it mannully. We run the old version of checkpoint this time!")
        use_safetensor = False
